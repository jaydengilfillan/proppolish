...
DO: Transform this daytime shot into a realistic night twilight scene. Set the sky to the sky shown in the second reference image. First, carefully inspect the ORIGINAL daytime photo and identify only the light fixtures that are ACTUALLY, PHYSICALLY visible in it — a window light, a downlight, a wall sconce, a path/step light, an eave light, etc. Then switch on ONLY those specific fixtures, in exactly the position they already exist. Do not invent a fixture anywhere just because that's a typical place a light would be installed on a similar house (e.g. under an eave, beside a front door, spaced along a wall) — if a fixture is not visible in the original photo, it does not get added or lit, full stop. The one narrow exception: small, non-fixed garden/landscape lights (e.g. solar stake lights or small spike lights along a garden bed or path) may be added tastefully and sparingly even if not clearly visible in the daytime shot, since these are common, low-impact, easily-removable accessories — this exception does NOT extend to anything fixed to the building itself. Keep the house, landscaping, and camera angle exactly the same. Maintain a natural, balanced lighting between the sky and the lit building — most of the building's surface should remain in soft ambient dusk light, not lit up like every wall has its own light source. If a pool is visible, illuminate the pool water a natural light blue. Cinematic, high-end real estate twilight look — photorealistic, no stylisation.
...
- If there IS a marker: that marker is your guide for exactly which property is the subject — do not guess, use whichever property it's pointing at/around. Leave the marker itself completely untouched — do not remove, move, resize, redraw or restyle it in any way; it must appear in the output exactly as it does in the original photo, in the same position. Using that marker as your guide, follow this instead of lighting every building the same way:
  - Fully light the marked property as described above — every one of its own real fixtures switched on, following the same no-fabrication rule above (only fixtures actually visible get switched on).
...
ABSOLUTELY DO NOT (this is a legal requirement): add any light fixture, lamp, downlight, wall sconce, uplight, string light or illuminated feature that is not physically present in the original photograph — this specifically includes the two most common mistakes to avoid: inventing new downlights/spotlights under eaves that don't actually exist there, and inventing new wall-mounted lights/sconces spaced along walls that aren't in the original photo; do not wash whole sections of wall in light or make the building look like it has more fixtures than it actually does; add, remove, duplicate or move any furniture, landscaping, vehicle, person or object; change the camera angle, framing, composition, zoom or perspective; change the building's structure, walls, windows, doors, roofline, or any permanent feature; alter neighbouring buildings, fences, power lines or structures; light up the properties immediately next to a marked subject property in a way that competes with it for attention; remove, move, resize, redraw or restyle any marker, pin, circle, arrow or border identifying the subject property. Preserve the property's true architecture, layout and every permanent feature EXACTLY as photographed — only the sky, the ambient/exterior lighting from EXISTING fixtures, and (optionally) a few small non-fixed garden lights may change.
