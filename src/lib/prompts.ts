...
DO: Transform this daytime shot into a realistic night twilight scene, a premium, high-end "luxury listing" hero shot — this should look rich, warm and inviting, not flat or underwhelming. Set the sky to the sky shown in the second reference image. WINDOWS: every window on the property should read as lit from inside with a warm, confident, inviting glow, the way real houses actually look at dusk with their interior lights on — this is the standard, expected look for a twilight listing photo, so light windows properly and warmly, not dimly or timidly; do not leave windows dark just because you can't see a literal light fixture through them in a DAYTIME photo — you never can, that's normal, light them anyway. ALL EXTERIOR LIGHTING (downlights, eave lights, wall sconces, path/step lights, garden/landscape spike or stake lights, letterbox/mailbox lights, illuminated house numbers or signage — literally anything outside that isn't a window): be strict here, no exceptions. This includes NOT making a letterbox, house number or sign glow, backlit or illuminated unless it is genuinely already lit that way in the original daytime photo (visible as an actual light fixture) — a plain daytime letterbox/number stays a plain, unlit letterbox/number at twilight, just under the same ambient light as the rest of the property. First inspect the ORIGINAL daytime photo and identify only the fixtures whose physical body is actually visible in it — including small garden spike/stake lights, which are often visible in daylight as small stems or heads poking out of a garden bed or along a path even when switched off. Then switch on ONLY those specific fixtures, in exactly the position they already exist, and nothing else. Do not invent ANY exterior fixture — including garden/landscape lights — anywhere it isn't already visible in the original photo, no matter how typical or tasteful it would look on a similar property (e.g. lining a driveway, dotting a garden bed, marking a path). If you cannot see a fixture's physical body in the original daytime photo, it does not get added, full stop — there is no allowance for adding new garden lights that weren't there. Keep the house, landscaping, and camera angle exactly the same. To be clear, the restraint above is about not INVENTING new exterior fixture locations — it is not an instruction to dim things down: windows and any real fixtures that are there should still be lit brightly and confidently enough to produce a proper premium twilight look, with a natural, balanced glow between the sky and the lit building. PROPERTY BRIGHTNESS: this is a hero shot — every part of the subject property must stay clearly, brightly visible, never dark, muddy or silhouetted, even where there's no real fixture nearby to switch on. That means the building's own physical surfaces (brick, render, cladding, garage doors, trim, roofline), AND any other key part of the property in frame — stairs, decking, paths, a pool area, a jetty or water's edge, outdoor entertaining areas — must all read clearly, the way a real twilight photographer lifts the whole scene with soft ambient fill light so nothing falls into darkness, rather than only the areas near an actual fixture being visible. This is ambient brightness only, not a light source — it must NOT be achieved by adding a fixture, glow, or illuminated feature that isn't really there (that's still against the rule above); it's simply that the overall exposure should stay soft and even across the whole property so every area is clearly visible, the same way it would be in the fading ambient light just after sunset, well before it's fully dark. The glowing windows and sky colour are not enough on their own — the physical property itself, in every area, needs to read bright and clear too. If a pool is visible, illuminate the pool water a natural light blue. Push the overall grade toward a premium, editorial real estate campaign look, not a plain snapshot with the sky swapped: a richer, fuller sky gradient (still the same colours and cloud pattern from the reference image, just with more depth and richness rather than looking pale or washed out), a warm glow/bloom immediately around each lit window and fixture that bounces gently onto nearby surfaces the way real light does, deeper and more cinematic contrast between the warm lit elements and the cool dusk ambience, and true, rich colour in the building's real materials (brick, render, timber, roofing) and the landscaping — nothing should look flat, pale or washed out. Cinematic, high-end real estate twilight look — photorealistic, no stylisation.
...
ABSOLUTELY DO NOT (this is a legal requirement): add any EXTERIOR light fixture, lamp, downlight, wall sconce, uplight, garden/landscape spike or stake light, string light or illuminated feature that is not physically present in the original photograph (this restriction does NOT apply to windows — every window should be lit warmly as described above, that's expected) — this specifically includes the most common mistakes to avoid: inventing new downlights/spotlights under eaves that don't actually exist there, inventing new wall-mounted lights/sconces spaced along walls that aren't in the original photo, inventing new garden/landscape lights along a driveway, path or garden bed that aren't actually visible in the original photo, and making a letterbox, house number or sign glow/backlit/illuminated when it wasn't lit that way in the original photo — none of these are an exception to this rule, they must already be visible/lit in the original photo just like any other fixture; do not wash whole sections of wall in light or make the building look like it has more exterior fixtures than it actually does; leave every window dark or dim — that is just as wrong as inventing fake fixtures and makes the result look weak and unfinished; add, remove, duplicate or move any furniture, landscaping, vehicle, person or object; change the camera angle, framing, composition, zoom or perspective; change the building's structure, walls, windows, doors, roofline, or any permanent feature; alter neighbouring buildings, fences, power lines or structures; light up the properties immediately next to a marked subject property in a way that competes with it for attention; remove, move, resize, redraw or restyle any marker, pin, circle, arrow or border identifying the subject property; leave the building's facade, or any other key area of the subject property (stairs, decking, paths, pool, water's edge etc.), dark, muddy or underexposed — everything must stay brightly and clearly visible via soft ambient fill as described above, this is a hero shot, and this must be achieved through overall exposure, never by inventing a fixture or glow that isn't really there; add any garden/landscape lights that are not already visible in the original photo, anywhere in the frame, in any quantity. Preserve the property's true architecture, layout and every permanent feature EXACTLY as photographed — only the sky, all windows (lit warmly), and the exterior lighting from EXISTING fixtures (including existing garden lights, if visible) may change.
 ...
 */
export const OPENAI_DETAIL_PRESERVATION_INSTRUCTION = `Preserve fine detail everywhere in the frame, especially in busy or complex areas — distant buildings, skylines, foliage, roofing, brickwork, architectural trim, and flooring (timber grain, floorboard lines, tile pattern and grout, carpet texture). Do not blur, soften, smooth over, simplify, or regenerate these areas at lower fidelity than the rest of the image. Match the sharpness and level of detail of the original photograph as closely as possible: the result should never look softer, blurrier, or less detailed than the input — only cleaner and better lit, never lower resolution or less crisp.`;
