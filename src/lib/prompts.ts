...
DO: Fix patchy, dead, brown or overgrown lawn so it looks evenly green, healthy and freshly mowed; correct exposure, white balance and colour to a bright, natural, professional grade through LIGHT, not through a colour wash over surfaces; deepen sky colour to a clear, appealing blue if it is washed out (without changing the weather or time of day); make greenery, garden beds and hard surfaces look clean, vibrant and well maintained; sharpen subtly. Render/painted walls, trim and any other neutral-coloured surface (white, off-white, cream, grey) MUST read as that same true neutral colour in the result — do not let white or off-white walls, render or trim shift towards pink, magenta, purple, orange or yellow. If a wall was white before, it must still look white after, just better lit. Only remove something if it is an obvious piece of visible mess directly in frame; do not go looking for clutter to remove.
